...
<script type="text/javascript">
	...
	function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>positionConfig/toAdd.do';
		 diag.Width = 600;
		 diag.Height = 500;
	...
	function edit(AP_ID){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑";
		 diag.URL = '<%=basePath%>positionConfig/toEdit.do?ID='+AP_ID;
		 diag.Width = 600;
		 diag.Height = 500;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	function del(AP_ID){
		...
		if(flag){
			top.jzts();
			var url = "<%=basePath%>positionConfig/del.do?ID="+AP_ID+"&guid="+new Date().getTime();
	...
	function test(AP_ID){
		...
		if(flag){
			top.jzts();
			var url = "<%=basePath%>positionConfig/test.do?ID="+AP_ID+"&guid="+new Date().getTime();
			$.get(url,function(data){
				if("success" == data.result){
					alert("测试成功！");
					top.jzts();
	...
	<%-- function checkIp(strIp,strPort){
			top.jzts();
			var url = "<%=basePath%>positionConfig/checkIp.do?ip="+strIp+"&port="+strPort;
			$.get(url,function(data){
				if("success" == data.result){
					alert("连接成功");
				}else{
...
<div class="row-fluid">
	...
	<!-- 检索  -->
	<form action="<%=basePath%>/positionConfig.do" method="post" name="userForm" id="userForm">
	...
	<table id="table_report" class="table table-striped table-bordered table-hover">
		...
		<tr>
			...
			<th class='center'>空箱</th>
			<th class='center'>满箱</th>
			<th class='center'>待清洗空箱</th>
			<th class='center'>可使用空箱</th>
			<th class='center'>LED</th>
			<th class='center'>报警状态</th>
		...
		<c:choose>
			<c:when test="${not empty varList}">
				...
				<tr>
					...
					<td class='center'>${var.PositionCode}</td>
					<td class='center'>${var.IP}</td>
					<td class='center'>${var.PORT}</td>			
					<td class="center">${var.PositionInfo}</td>
					<%-- <td class="center">${var.State}</td> --%>
					<td class="center">${var.EmptyState}</td>
					<td class="center">${var.FullState}</td>
					<td class="center">${var.DurtyState}</td>
					<td class="center">${var.CanUseState}</td>
					<td class="center">${var.Led}</td>
					<td class="center">${var.AlertLight}</td>
					...
					<td style="width: 155px;">
						...
						<a class='btn btn-mini btn-pink' title="测试"  onclick="test('${var.ID }')"><i class='icon-bolt'></i></a>
						<%-- <a class='btn btn-mini btn-danger' title="测试IP"  onclick="checkIp('${var.IP }','${var.PORT }')"><i class='icon-trash'></i></a> --%>
					</td>
				</tr>
				</c:forEach>
			</c:when>
