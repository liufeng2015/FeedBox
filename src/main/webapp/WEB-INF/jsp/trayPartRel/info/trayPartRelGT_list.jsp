...
<script type="text/javascript">
	...
	function getAllPartData(strId,strFullNumber){
		...
		$.ajax({  
		    type : "post",  
		    url : url,  
		    data : "State=未使用&TrayId="+strId+"&FullNumber="+strFullNumber+"&Address=${pd.Address}",
		    async : false,//取消异步  
		    success : function(data){
		    	//选中托盘中所有容纳零件总和
		    	var FullNumber = 24;
				...
				for(var i = listData.length - 1;i >= 0;i--){
			        ...
			        if(FullNumber>=size){
			        	var trHTML='<tr>'
							...
							+'  <td><a class="btn btn-mini btn-danger" title="删除"  onclick="javascript:doDelete(\''+strId+'\')">删除</a></td>';
				...
				<form action="<%=basePath%>/trayPartRelInfo.do" method="post"	name="userForm" id="userForm">
					...
					<table>
						<tr>
							...
							<td><font color="#808080">班组：</font></td>
							<td height="50px" ><select id="ROW" name="ROW" placeholder="Choose a Country..."  style="height: 50px" >
					...
					<input type="button" value="绑定并打印" class="btn btn-app btn-success" onclick="bingingTrayPart()"  style="width: 150px;height: 100px" />
